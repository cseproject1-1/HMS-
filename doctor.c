...
void view_my_patients(const char* doctor_id) {
    ...
    if(!found) printf("No patients assigned to you.\n");
}
...
void update_patient_record(const char* doctor_id){
    ...
    for(int i=0;i<patient_count;i++){
        if(strcmp(patients[i].patient_id, patient_id)==0&&strcmp(patients[i].doctor_id,doctor_id)==0){
...
void search_patient(void) {
    ...
    for(int i=0;i<patient_count;i++){
        ...
        if ((search_choice==1 &&strcmp(patients[i].patient_id,search_term)==0)||(search_choice==2&&strstr(temp_name,search_term)!=NULL)){
            printf("Patient ID: %s\n", patients[i].patient_id);
            printf("Name: %s\n", patients[i].name);
            printf("Age: %d\n", patients[i].age);
            printf("Gender: %c\n", patients[i].gender);
